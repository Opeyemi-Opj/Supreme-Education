...
const PopupMenu = ({ onClose }) => {  
  ...
  const [activeIndex, setActiveIndex] = useState(null); // Clicked title
  const [hoverIndex, setHoverIndex] = useState(null);   // Hovered title
...
<div className="content-center pl-10">
  <div className="border-r-2 border-[#C4C4C4] space-y-[10%]">
    {titles.map((title, index) => (
      <h2
        key={index}
        className={`font-cormorant font-bold text-[32px] leading-[38.75px] cursor-pointer transition duration-300 ${
          hoverIndex === index
            ? "text-[#DD3333]" // Hovering → Red
            : activeIndex === index && hoverIndex === null
            ? "text-[#DD3333]" // Active but only when no hover
            : "text-[#292D32]" // Default
        }`}
        style={{ width: "fit-content", willChange: "color" }}
        onMouseEnter={() => {
          setHoverIndex(index);
      ...
      <div className="pt-10 bg-[#FFFFF]">
        <div className="flex gap-2 justify-start pl-5">
          <button
            className="font-raleway text-[#FFFFFF] text-[14px] font-medium leading-[16.44px] bg-[#006011] border border-[#C6C5C5] rounded-[8px] px-[24px] py-[12px] items-center gap-5 transition duration-300
              hover:bg-[#C8352E] "
          >
            Portal
          </button>
          ...
          <div className="relative flex items-center pr-[5%]">
            ...
            <input
              ...
              className={`absolute right-0 top-0 h-10 max-w-0 px-4 border border-gray-300 rounded-full transition-all duration-300 ${
                isSearchOpen ? "max-w-[130px] opacity-100 px-4" : "max-w-0 opacity-0 px-0"
              }`}
            />
          </div>
          ...
          {contentLinks.map((links, index) => (
            <div
              ...
              className={`mt-[50px] space-y-[5%] ${
                activeIndex === index || hoverIndex === index ? "block" : "hidden"
              }`}
            >
              {links.map((link, idx) => (
                <p
